...
- **Filtro**: `JwtFilterRequest` valida encabezado `Authorization: Bearer <token>`, carga el `UserDetails` con authorities reales y puebla el `SecurityContext` listo para anotaciones `@PreAuthorize`. 【F:back-costa/src/main/java/com/ferrisys/config/security/filter/JwtFilterRequest.java†L1-L59】
- **Authorities**: `CustomUserDetailsService` construye authorities `ROLE_<NOMBRE>` y `MODULE_<SLUG>` a partir del rol y módulos activos del usuario (consultados vía `RoleModuleRepository`). 【F:back-costa/src/main/java/com/ferrisys/config/security/CustomUserDetailsService.java†L1-L74】
- **Method security**: habilitada con `@EnableMethodSecurity` y usada en inventario para exigir `MODULE_INVENTORY` o rol `ADMIN` en listados. 【F:back-costa/src/main/java/com/ferrisys/config/security/MethodSecurityConfig.java†L1-L8】【F:back-costa/src/main/java/com/ferrisys/controller/InventoryController.java†L1-L56】
